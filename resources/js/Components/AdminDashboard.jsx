...
export default function AdminDashboard() {
    ...
    // Prepara el borrado (Abre el modal rojo)
    const requestDeleteEmployee = (dni, name) => {
        setEmployeeToDelete({ dni, name });
        setShowDeleteConfirm(true);
    };

    // Ejecuta el borrado real en la base de datos
        ...
        fetch(`/empleados/${employeeToDelete.dni}`, {
            ...
            .then((res) => {
                ...
                setRawEmployees((prev) =>
                    prev.filter((emp) => emp.dni !== employeeToDelete.dni),
                );
    ...
    const handleUpdateEmployee = (empleadoActualizado) => {
        ...
        setRawEmployees((prev) =>
            prev.map((emp) =>
                emp.dni === empleadoActualizado.dni ? empleadoActualizado : emp,
            ),
        );
        ...
        <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-12 relative">
            ...
            <main className="max-w-[1600px] mx-auto px-6 mt-8 flex flex-col lg:flex-row gap-8">
                ...
                <aside className="w-full lg:w-1/3 xl:w-1/4 flex flex-col gap-6">
                    ...
                    >
                        <i className="fa-solid fa-bullhorn text-lg"></i> MANEJAR
                        ALERTAS
                    </a>
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-5">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            >
                                ...
                                <span
                                    className={`px-2 py-0.5 rounded-full text-xs ${selectedRole === "Todos" ? "bg-blue-100 text-blue-800" : "bg-slate-100 text-slate-500"}`}
                                >
                                    {rawEmployees.length}
                                </span>
                            </button>
                            ...
                            {roles.map((role) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-2">
                                            <i
                                                className={`fa-solid ${style.icon} w-5 text-center`}
                                            ></i>{" "}
                                            {role}
                                        </div>
                                        <span
                                            className={`px-2 py-0.5 rounded-full text-xs ${isActive ? "bg-white/60" : "bg-slate-100 text-slate-500"}`}
                                        >
                                            {count}
                                        </span>
                                    </button>
            ...
            <EmployeeDrawer
                ...
                onDeleteRequest={(dni, name) =>
                    requestDeleteEmployee(dni, name)
                }
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-fade-in">
                    ...
                    <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden text-center p-8">
                        ...
                        <p className="text-slate-500 text-sm mb-8">
                            ...
                            del sistema. Esta acción revocará su acceso y no se
                            puede deshacer.
                        </p>
                        <div className="flex gap-3">
                            <button
                                onClick={() => setShowDeleteConfirm(false)}
                                className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-600 py-3.5 rounded-xl text-sm font-bold transition-colors"
                            >
                                CANCELAR
                            </button>
                            <button
                                onClick={handleDeleteEmployee}
                                className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3.5 rounded-xl text-sm font-black shadow-md transition-colors flex justify-center items-center gap-2"
                            >
