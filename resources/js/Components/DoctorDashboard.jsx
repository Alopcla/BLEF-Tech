...
export default function DoctorDashboard() {
    ...
    useEffect(() => {
        fetch('/api/medico/datos')
            ...
            .catch(err => console.error("Error cargando datos médicos:", err));
    }, []);
    ...
    if (isLoading) {
        return <div className="min-h-screen flex justify-center items-center bg-slate-50"><i className="fa-solid fa-circle-notch fa-spin text-4xl text-teal-600"></i></div>;
    }
                    ...
                    <div className="bg-white px-5 py-4 rounded-3xl shadow-sm border border-slate-200 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-xs font-black text-slate-400 uppercase tracking-widest">
                                Zona: {doctorInfo?.zone_id ? `${doctorInfo.zone_id}` : 'General'}
                            </p>
...
function MedicalRecordDrawer({ isOpen, onClose, animal }) {
    ...
    return (
        <>
            ...
            <aside className={`fixed top-0 right-0 h-full w-full max-w-lg bg-white shadow-2xl z-50 transform transition-transform duration-500 ease-out flex flex-col border-l border-slate-200 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>

                <div className="bg-slate-50 p-6 relative border-b border-slate-200 shrink-0">
                    <button onClick={handleClose} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-400 hover:text-red-500 transition-all shadow-sm">
                    ...
                    {!isAddingMode ? (
                        <div className="space-y-6">
                            ...
                            <div className="space-y-4">
                                {animal.medical_records?.map(record => (
                                    <div key={record.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="flex items-center gap-2">
                                                <i className="fa-solid fa-calendar-day text-teal-500"></i>
                                                <span className="text-xs font-bold text-slate-600 font-mono">{record.date}</span>
                                            </div>
                                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">DNI Médico: {record.employee_dni}</span>
                                        </div>
                                        <div className="space-y-3">
                                            ...
                                            <div className="bg-white p-3 rounded-xl border border-slate-100">
                                                <p className="text-[10px] font-black text-teal-600 uppercase tracking-widest mb-1">Tratamiento</p>
                        ...
                        <div className="space-y-5 animate-fade-in">
                            <h3 className="text-xs font-black text-teal-600 uppercase tracking-widest border-b border-teal-100 pb-2 mb-4">
                                <i className="fa-solid fa-file-medical mr-2"></i>Nueva Evaluación Médica
                            </h3>

                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Diagnóstico Clínico</label>
                                <textarea
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-slate-400 uppercase">Tratamiento Recetado</label>
                                <textarea
                ...
                <div className="p-6 border-t border-slate-100 bg-slate-50 shrink-0">
                    {!isAddingMode ? (
                        ...
                        >
                            <i className="fa-solid fa-stethoscope"></i> AÑADIR NUEVO DIAGNÓSTICO
                        </button>
                    ) : (
                        <div className="flex gap-3">
                            <button onClick={handleSave} className="flex-1 bg-teal-600 text-white px-4 py-3.5 rounded-xl text-sm font-black shadow-md hover:bg-teal-700 flex justify-center items-center gap-2 transition-colors">
                                <i className="fa-solid fa-floppy-disk"></i> GUARDAR FICHA
                            </button>
                            <button onClick={() => setIsAddingMode(false)} className="flex-[0.5] bg-white border border-slate-200 text-slate-500 px-4 py-3.5 rounded-xl text-sm font-bold hover:bg-slate-100 transition-colors">
                                CANCELAR
                            </button>
                        </div>
                    )}
