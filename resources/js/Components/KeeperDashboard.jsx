...
export default function KeeperDashboard() {
    ...
    const fedAnimals = useMemo(
        () => animals.filter((a) => a.last_fed_date === today),
        [animals, today],
    );
                    ...
                    <div className="bg-white px-5 py-4 rounded-3xl shadow-sm border border-slate-200 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-bold text-slate-800">
                                {keeperInfo?.name} {keeperInfo?.surname}
                            </p>
                        </div>
                    </div>
                    ...
                    {animals.length === 0 ? (
                        <div className="bg-white p-12 text-center rounded-3xl border border-slate-200 shadow-sm">
                            ...
                            <p className="text-slate-500 font-bold">
                                No tienes animales asignados en tu zona para
                                alimentar.
                            </p>
                        </div>
                    ) : (
                                ...
                                <div>
                                    <div className="flex items-center gap-4 mb-6">
                                        <h2 className="text-sm font-black text-amber-500 uppercase tracking-widest whitespace-nowrap">
                                            ...
                                            Pendientes ({pendingAnimals.length})
                                        </h2>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                        {pendingAnimals.map((animal) => (
                                            <div
                                                key={animal.id}
                                                onClick={() =>
                                                    openAnimalCare(animal)
                                                }
                                                className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm hover:shadow-xl transition-all cursor-pointer group"
                                            >
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 opacity-75 hover:opacity-100 transition-opacity">
                                        {fedAnimals.map((animal) => (
                                            <div
                                                ...
                                                className="bg-white rounded-3xl p-6 border border-emerald-100 shadow-sm hover:shadow-md transition-all cursor-pointer group"
                                            >
                                                <div className="flex justify-between items-start mb-4">
...
function KeeperDrawer({ isOpen, onClose, animal, today }) {
    ...
    return (
        <>
            ...
            <aside
                className={`fixed top-0 right-0 h-full w-full max-w-lg bg-white shadow-2xl z-50 transform transition-transform duration-500 flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"}`}
            >
                <div className="bg-slate-50 p-6 relative border-b border-slate-200">
                    <button
                        onClick={onClose}
                        className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-400"
                    >
                ...
                <div className="p-6 space-y-6 flex-grow">
                    <div className="bg-lime-50 border border-lime-100 rounded-2xl p-5">
                        <h3 className="text-[10px] font-black text-lime-600 uppercase tracking-widest mb-2">
                            Ración Diaria
                        </h3>
                        <p className="text-xl font-black text-lime-900">
                            {animal.food_ration || "No especificada"}
                        </p>
                    </div>

                    <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5">
                        <h3 className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2 text-left">
                            Tipo de Dieta
                        </h3>
                        ...
                        {animal.last_fed_date ? (
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex justify-between">
                                ...
                                <span className="text-[10px] font-black text-slate-400">
                                    ID CUIDADOR: {animal.last_fed_by}
                                </span>
                            </div>
                        ) : (
                            <p className="text-sm text-slate-400 italic text-left">
                ...
                <div className="p-6 bg-slate-50 border-t border-slate-100">
                    ...
                    >
                        {hasFedToday ? (
                            ...
                            <>
                                <i className="fa-solid fa-utensils"></i> MARCAR
                                COMO COMPLETADO
                            </>
                        )}
                    </button>
