...
export default function KeeperDashboard() {
    ...
    // --- DIVISIÓN DE ANIMALES (PENDIENTES Y ALIMENTADOS) ---
    const fedAnimals = useMemo(() => animals.filter(a => a.last_fed_date === today), [animals, today]);
    const pendingAnimals = useMemo(() => animals.filter(a => a.last_fed_date !== today), [animals, today]);
    ...
    if (isLoading) {
        return <div className="min-h-screen flex justify-center items-center bg-slate-50"><i className="fa-solid fa-circle-notch fa-spin text-4xl text-lime-600"></i></div>;
    }
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-12">
            <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
                <div className="max-w-[1600px] mx-auto px-6 py-4 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        <div className="bg-lime-500 text-white w-10 h-10 rounded-xl flex items-center justify-center text-xl shadow-md">
                            <i className="fa-solid fa-leaf"></i>
                        </div>
                        <h1 className="text-xl font-black tracking-tight text-slate-800">
                            Zoo<span className="text-lime-500">Pro</span> Keeper
                        </h1>
                    </div>
                    ...
                    <div className="bg-white px-5 py-4 rounded-3xl shadow-sm border border-slate-200 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-xs font-black text-slate-400 uppercase tracking-widest">
                                Zona: {keeperInfo?.zone_id || 'Admin'}
                            </p>
                            <p className="text-sm font-bold text-slate-800">{keeperInfo?.name} {keeperInfo?.surname}</p>
                        </div>
                    </div>
                ...
                <section className="flex-1">
                    {animals.length === 0 ? (
                        <div className="bg-white p-12 text-center rounded-3xl border border-slate-200 shadow-sm">
                            ...
                            <p className="text-slate-500 font-bold">No tienes animales asignados en tu zona para alimentar.</p>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-10">
                            ...
                            {pendingAnimals.length > 0 && (
                                <div>
                                    <div className="flex items-center gap-4 mb-6">
                                        <h2 className="text-sm font-black text-amber-500 uppercase tracking-widest whitespace-nowrap">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                        {pendingAnimals.map(animal => (
                                            <div key={animal.id} onClick={() => openAnimalCare(animal)} className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm hover:shadow-xl transition-all cursor-pointer group">
                                                <div className="flex justify-between items-start mb-4">
                                                    ...
                                                    <span className="bg-amber-50 text-amber-600 border border-amber-200 px-3 py-1 rounded-full text-[10px] font-black uppercase animate-pulse">Pendiente</span>
                                                </div>
                                                <h3 className="text-xl font-black text-slate-800">{animal.common_name}</h3>
                                                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{animal.species}</p>
                                                <div className="pt-4 border-t border-slate-100 flex justify-between items-center text-xs">
                                                    <span className="font-bold text-slate-500 italic"><i className="fa-solid fa-scale-balanced mr-1"></i> {animal.food_ration || 'Sin ración'}</span>
                                                    <span className="text-lime-600 font-black">GESTIONAR <i className="fa-solid fa-arrow-right ml-1"></i></span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            {fedAnimals.length > 0 && (
                                <div>
                                    <div className="flex items-center gap-4 mb-6">
                                        <h2 className="text-sm font-black text-emerald-500 uppercase tracking-widest whitespace-nowrap">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 opacity-75 hover:opacity-100 transition-opacity">
                                        {fedAnimals.map(animal => (
                                            <div key={animal.id} onClick={() => openAnimalCare(animal)} className="bg-white rounded-3xl p-6 border border-emerald-100 shadow-sm hover:shadow-md transition-all cursor-pointer group">
                                                <div className="flex justify-between items-start mb-4">
                                                    <div className="w-14 h-14 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-500">
                                                        <i className="fa-solid fa-paw text-2xl"></i>
                                                    </div>
                                                    <span className="bg-emerald-50 text-emerald-600 border border-emerald-200 px-3 py-1 rounded-full text-[10px] font-black uppercase">Finalizado</span>
                                                </div>
                                                <h3 className="text-xl font-black text-slate-800">{animal.common_name}</h3>
                                                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{animal.species}</p>
                                                <div className="pt-4 border-t border-slate-100 flex justify-between items-center text-xs">
                                                    <span className="font-bold text-slate-500 italic"><i className="fa-solid fa-scale-balanced mr-1"></i> {animal.food_ration || 'Sin ración'}</span>
                                                    <span className="text-emerald-600 font-black">VER FICHA <i className="fa-solid fa-arrow-right ml-1"></i></span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
            ...
            <KeeperDrawer isOpen={isDrawerOpen} onClose={() => setIsDrawerOpen(false)} animal={selectedAnimal} today={today} />
        </div>
    );
}
...
function KeeperDrawer({ isOpen, onClose, animal, today }) {
    ...
    const hasFedToday = animal.last_fed_date === today;
        ...
        <>
            ...
            <aside className={`fixed top-0 right-0 h-full w-full max-w-lg bg-white shadow-2xl z-50 transform transition-transform duration-500 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="bg-slate-50 p-6 relative border-b border-slate-200">
                    <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-400"><i className="fa-solid fa-xmark"></i></button>
                    <div className="flex items-center gap-4 mt-2">
                        <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl bg-lime-50 text-lime-500 shadow-inner"><i className="fa-solid fa-utensils"></i></div>
                        <div>
                            <h2 className="text-2xl font-black text-slate-800">{animal.common_name}</h2>
                ...
                <div className="p-6 space-y-6 flex-grow">
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5">
                        ...
                        <p className="text-lg font-bold text-blue-900">{animal.diet}</p>
                    </div>
                    ...
                    <div className="border-t border-slate-100 pt-6">
                        ...
                        {animal.last_fed_date ? (
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex justify-between">
                                <span className="text-sm font-bold text-slate-700"><i className="fa-solid fa-calendar mr-2"></i>{animal.last_fed_date}</span>
                                <span className="text-[10px] font-black text-slate-400">ID CUIDADOR: {animal.last_fed_by}</span>
                            </div>
                ...
                <div className="p-6 bg-slate-50 border-t border-slate-100">
                    ...
                    >
                        {hasFedToday ? <><i className="fa-solid fa-check-double"></i> TAREA FINALIZADA</> : <><i className="fa-solid fa-utensils"></i> MARCAR COMO COMPLETADO</>}
                    </button>
                </div>
            </aside>
        </>
