...
function StepEmail({ onCodeSent }) {
    ...
    const handleSend = async () => {
        ...
        setError("");
        try {
            const res = await fetch("/api/compras/send-code", {
                method:  "POST",
    ...
    return (
        <div className="relative group max-w-2xl mx-auto">
            <div className="absolute -inset-1 bg-gradient-to-r from-[#D9C8A1]/20 to-transparent rounded-[2rem] blur opacity-25 group-hover:opacity-50 transition" />
            <div className="relative flex flex-col md:flex-row gap-4 p-2 bg-[#1A2E1A]/40 backdrop-blur-xl border border-white/10 rounded-[2rem]">
                <div className="flex-grow relative">
                    <i className="fa-solid fa-envelope absolute left-6 top-1/2 -translate-y-1/2 text-[#D9C8A1]/40" />
                    <input
                        type="email"
                        value={email}
            ...
            <p className="text-white/20 text-[10px] text-center mt-4 tracking-widest uppercase">
...
function StepCode({ email, onVerified, onBack }) {
    ...
    const handleVerify = async () => {
        ...
        setError("");
        try {
            const res = await fetch("/api/compras/verify-code", {
                method:  "POST",
                headers: { "Content-Type": "application/json", "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content },
            ...
            onVerified(json.access_token, email);
        } catch (e) {
            setError(e.message);
        }
        setLoading(false);
    ...
    const handleResend = async () => {
        await fetch("/api/compras/send-code", {
            method:  "POST",
            headers: { "Content-Type": "application/json", "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content },
            body:    JSON.stringify({ email }),
        });
    ...
    return (
        <div className="max-w-md mx-auto text-center space-y-8">
            {/* Icono */}
            <div className="w-20 h-20 mx-auto rounded-full bg-[#D9C8A1]/10 border border-[#D9C8A1]/20 flex items-center justify-center">
                <i className="fa-solid fa-shield-halved text-3xl text-[#D9C8A1]" />
            </div>

            <div>
                <h2 className="text-white font-bold text-xl">Verifica tu identidad</h2>
                <p className="text-white/40 text-sm mt-2">
                    Hemos enviado un código de 6 dígitos a<br />
                    <span className="text-[#D9C8A1]">{email}</span>
                </p>
            </div>

            {/* Input del código */}
            <div className="relative group">
                <div className="absolute -inset-1 bg-gradient-to-r from-[#D9C8A1]/20 to-transparent rounded-[2rem] blur opacity-25" />
                <div className="relative flex flex-col gap-4 p-2 bg-[#1A2E1A]/40 backdrop-blur-xl border border-white/10 rounded-[2rem]">
                    <div className="flex-grow relative">
                        <i className="fa-solid fa-key absolute left-6 top-1/2 -translate-y-1/2 text-[#D9C8A1]/40" />
                        <input
                            type="text"
                            inputMode="numeric"
                            maxLength={6}
                            ...
                            placeholder="······"
                            className="w-full pl-14 pr-6 py-4 bg-transparent text-white placeholder:text-white/20 text-center text-2xl tracking-[1rem] focus:outline-none font-mono"
                        />
                    </div>
                    <button
                        onClick={handleVerify}
                        disabled={code.length !== 6 || loading}
                        className="px-10 py-4 bg-[#D9C8A1] text-[#1A2E1A] rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-white transition disabled:opacity-40 disabled:cursor-not-allowed"
                    >
            ...
            <div className="flex items-center justify-center gap-6 text-[10px] uppercase tracking-widest">
                <button onClick={onBack} className="text-white/20 hover:text-white/50 transition">
                    ← Cambiar email
                </button>
                ...
                <button
                    ...
                    disabled={resent}
                    className="text-white/20 hover:text-[#D9C8A1] transition disabled:text-green-500/60 disabled:cursor-default"
                >
...
export default function MyOrders({ auth, initialEmail }) {
    ...
    // ── step === "verified" → contenido normal ──
    return (
        <div className="space-y-12 animate-fade-in">
            ...
            {loading && (
                <div className="flex justify-center py-20">
                    <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#D9C8A1]" />
                </div>
            )}
                    ...
                    {tab === "tickets" && (
                        data.tickets.length > 0 ? (() => {
                            ...
                            return Object.values(grouped).map((group, index) => {
                                ...
                                return (
                                    <div key={index} className="break-inside-avoid bg-[#1A2E1A]/60 border border-white/10 rounded-[2.5rem] p-8 hover:border-[#D9C8A1]/30 transition-all mb-6">
                                                ...
                                                >
                                                    ...
                                                    <div className="text-right flex items-center gap-4">
                                                        <div className="flex flex-col items-end">
                                                            <span className="text-white font-bold text-sm tracking-tight">{t.price}€</span>
                                                            <span className="text-[8px] text-[#D9C8A1] uppercase tracking-widest opacity-60">Individual</span>
                                                        </div>
                            ...
                            ? data.experiencias.map(e => (
                                <div key={e.id} className="break-inside-avoid bg-[#1A2E1A]/60 border border-white/10 rounded-[2rem] p-8 flex flex-col sm:flex-row gap-6 hover:border-[#D9C8A1]/30 transition-all group mb-6">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-white/40 text-xs mt-1"><i className="fa-solid fa-calendar-day mr-2" />{formatDate(e.reservation_date)}</p>
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <div className="w-16 h-16 rounded-xl overflow-hidden bg-black/40 border border-white/10 shrink-0">
                                                        {item.product?.image_url
                                                            ...
                                                            : <i className="fa-solid fa-box text-xl text-white/10" />
                                                        }
                                                    </div>
                                                    <div>
            ...
            <style jsx>{`
                ...
                .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(217,200,161,0.4); }
            `}</style>
        </div>
    );
}
