<header class="absolute w-full top-0 z-[100] flex justify-center pt-6 px-4">
    <div
        class="flex items-center gap-4 md:gap-8 bg-[#1A2E1A]/80 backdrop-blur-xl px-4 md:px-8 py-3 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.3)] border border-white/10 relative">
        ...
        <a href="/" class="relative z-10 transition-transform hover:scale-110 duration-300">
            <img src="{{ asset('LOGO.PNG') }}" class="w-12 md:w-14 drop-shadow-[0_0_8px_rgba(217,200,161,0.5)]"
                alt="Park Zoo">
        </a>
        ...
        <nav class="hidden lg:flex gap-2 text-sm xl:text-base font-parkzoo items-center relative z-10">
            @foreach($links as $link)
                <a href="{{ $link['url'] }}"
                    ...
                    <i
                        class="fa-solid {{ $link['icon'] }} text-xs opacity-70 group-hover:rotate-12 transition-transform"></i>
        ...
        <div class="hidden md:flex items-center gap-3 relative z-10">
            @if($authUser)
                <div class="relative group">
                    <button
                        class="text-[#D9C8A1] hover:text-white transition-colors p-2 flex items-center gap-2 outline-none">
                        <i class="fa-solid fa-circle-user text-2xl"></i>
                        <i
                            class="fa-solid fa-chevron-down text-[10px] opacity-50 group-hover:rotate-180 transition-transform duration-300"></i>
                    </button>

                    <div
                        class="absolute right-0 top-full mt-2 w-52 bg-[#1A2E1A] border border-white/10 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] py-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-[150] backdrop-blur-xl">
                        ...
                        @if(Auth::guard('employee')->check())
                            <a href="{{ route('dashboard') }}"
                                class="flex items-center gap-3 px-5 py-3 text-[#D9C8A1] hover:bg-white/5 transition-colors text-sm font-bold">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                class="w-full flex items-center gap-3 px-5 py-3 text-red-400 hover:bg-red-500/10 transition-colors text-sm font-bold text-left">
            ...
            @else
                <a href="{{ route('login') }}"
                    class="group flex items-center gap-2 border border-[#D9C8A1]/30 px-4 py-2 rounded-xl text-[#D9C8A1] hover:bg-[#D9C8A1] hover:text-[#1A2E1A] transition-all duration-500 text-sm font-bold">
        ...
        <button class="lg:hidden text-[#D9C8A1] p-2 hover:bg-white/10 rounded-lg transition-colors relative z-[110]"
            id="menu-toggle">
    ...
    <div id="mobile-menu"
        class="fixed inset-0 bg-[#1A2E1A]/95 backdrop-blur-2xl z-[90] translate-x-full transition-transform duration-500 ease-in-out lg:hidden">
        <div class="flex flex-col h-full pt-32 px-10 pb-12">
            <button id="menu-close"
                class="absolute top-8 right-8 w-10 h-10 flex items-center justify-center rounded-full border border-[#D9C8A1]/20 text-[#D9C8A1] hover:bg-[#D9C8A1]/10 transition-all duration-300">
            ...
            <nav class="flex flex-col gap-4">
                @foreach($links as $link)
                    <a href="{{ $link['url'] }}"
                        class="text-3xl font-bold text-[#D9C8A1] flex items-center justify-between py-4 border-b border-white/5">
            ...
            <div class="mt-auto space-y-4">
                @if($authUser)
                    <div class="grid grid-cols-1 gap-3">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                class="w-full flex items-center justify-between p-4 border border-red-500/20 rounded-2xl text-red-400 active:bg-red-500/5 transition-all">
                ...
                @else
                    <a href="{{ route('login') }}"
                        class="flex items-center justify-between p-4 border border-[#D9C8A1]/20 rounded-2xl text-[#D9C8A1] active:bg-white/5 transition-all mb-2">
                ...
                <a href="{{ route('payment.show') }}"
                    class="w-full py-5 bg-gradient-to-r from-[#F2C94C] to-[#F2994A] text-[#1A2E1A] rounded-2xl font-black text-center block text-lg uppercase tracking-widest shadow-xl">
                    Tickets
                </a>
            </div>
        </div>
